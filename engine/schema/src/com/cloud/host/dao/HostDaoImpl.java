...
public class HostDaoImpl extends GenericDaoBase<HostVO, Long> implements HostDao { //FIXME: , ExternalIdDao {
    ...
    public boolean updateState(Status oldStatus, Event event, Status newStatus, Host vo, Object data) {
        ...
        if (result == 0) {
            ...
            if (status_logger.isDebugEnabled()) {
                ...
                str.append("; New=[status=").append(newStatus.toString()).append(":msid=").append(newStatus.lostConnection() ? "null" : host.getManagementServerId()).append(":lastpinged=")
                .append(host.getLastPinged()).append("]");
    ...
    public HostVO findHostToOperateOnSnapshot(long snapshotId) {
        ...
        try {
            ...
            ResultSet rs = pstmt.executeQuery();
            if (!rs.next()) {
                throw new CloudRuntimeException(String.format("Could not find a host connected to a storage pool that is used to store the informed snapshot [snapshotId=%d]. ", snapshotId));
            }
